...
Run `npm test` and `npm run check` before publishing. Smoke-test all four city searches, mobile width, keyboard navigation, and the source-failure message. Run `node scripts/sync-houston.mjs` to refresh Houston manually.
