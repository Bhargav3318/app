...
PermitTap is a dependency-free static ES-module application. `src/data.js` creates bounded Socrata and CKAN queries, searches the Houston snapshot, and maps each publisher schema to one presentation shape. `src/app.js` owns form state, safe HTML rendering, loading and failure states, and official-source links.
